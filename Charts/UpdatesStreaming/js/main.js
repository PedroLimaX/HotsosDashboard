...
function Task(id,title, description, status = 'Pendiente', date){
  ...
  this.status = status;
  this.date = date;
  if(this.title=='') this.title = 'Desconocido';
  if(this.description=='') this.description = 'Desconocido';
  ...
  for(const task of taskLocal){
    ...
    element.innerHTML= `
      ...
      <div class='status-indicator'></div>
        <p class='title-txt'><span>Habitacion: </span>${task.title}</p>
        <p class='description-txt'><span>Version OS: </span>${task.description}</p>
        <p class='status-txt'>${task.status}</p>
        <p class='date-txt'><span>Última Actualización: </span>${task.date}</p>
        <div class='task-btns'>
...
editTask = (id) =>{
  let task = taskList[id];
    ...
    element.innerHTML= `
        <div class='status-indicator'></div>
        <p class='title-txt'><span>Habitacion: </span><input type='text' value='${task.title}' id='titleEdit' name='titleEdit'></p>
        <p class='description-txt'><span>Version OS: </span><input type='text' value='${task.description}' id='descriptionEdit' name='descriptionEdit'></p>
  ...
  changeStatus = (n) =>{
    ...
    updateTask = (id) =>{
        ...
        element.innerHTML= `
            <div class='status-indicator'></div>
            <div class='title-txt'><span>Habitacion: </span>${taskList[id].title}</div>
            <p class='description-txt'><span>Version OS: </span>${taskList[id].description}</p>
            <p class='status-txt'>${taskList[id].status}</p>
            <p class='date-txt'><span>Última Actualización: </span>${taskList[id].date}</p>
            <div class='task-btns'>
